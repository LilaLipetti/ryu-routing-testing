...
# 6. enable/disable routeset
# enable the route
# PUT /routes/enable/{route_id}
## curl -X PUT http://1.100.53.2:8080/routes/enable/1
#
# disables current active route
# PUT /routes/disable
## curl -X PUT http://1.100.53.2:8080/routes/disable/1
...
class RestRouterAPI(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        routing_requirements={'route_id':ROUTE_PATTERN}

        ### For routing topology data

        path='/routes/{route_id}'
        mapper.connect('routes', path, controller=RouterController,
        ...
        mapper.connect('routes', path, controller=RouterController,
                       ...
                       action='get_route_data',
                       conditions=dict(method=['GET']))

        mapper.connect('routes', path, controller=RouterController,                       
                       requirements=routing_requirements,
                       action='delete_route_data',
                       conditions=dict(method=['DELETE']))
                       
                       
        path='/routes/enable/{route_id}'
        mapper.connect('routes', path, controller=RouterController,
                       requirements=routing_requirements, 
                       action='activate_route_data',
                       conditions=dict(method=['PUT']))
                       
        path='/routes/disable'
        mapper.connect('routes', path, controller=RouterController,
                       action='disable_route_data',
                       conditions=dict(method=['PUT']))
        ### end routing topology data
    ...
    def port_status_handler(self, ev):
        #msg = ev.msg
        #dp = msg.datapath
        #dpid_str = {'sw_id': dpid_to_str(dp.id)}
        #port = msg.desc
        #reason = msg.reason
        ...
        if reason is dp.ofproto.OFPPR_ADD:            
            RouterController.port_add_handler(ev)
       
        elif reason is dp.ofproto.OFPPR_DELETE:
            RouterController.port_delete_handler(ev)
        
        else:
            assert reason is dp.ofproto.OFPPR_MODIFY
            if link_down_flg:
                RouterController.link_down_handler(ev)
                
            else:
                RouterController.link_up_handler(ev)
...
class RouterController(ControllerBase):
    ...
    @classmethod
    def set_route_id(cls,routeid):
        cls.current_active_route=routeid
    ...
    @classmethod
    def port_add_handler(cls,ev):

        msg = ev.msg
        dp = msg.datapath
        dp_id = dp.id
        dpid_str = {'sw_id': dpid_to_str(dp_id)}
        port = msg.desc
        reason = msg.reason
        link_down_flg = port.state & 0b1
        ...
        if dp_id in cls._ROUTER_LIST:
            cls._LOGGER.info('OFPPortStatus [port=%d] Port added.',
                             port.port_no, extra=dpid_str)
            router = cls._ROUTER_LIST[dp_id]

    @classmethod
    def port_delete_handler(cls,ev):
        ...
        if dp_id in cls._ROUTER_LIST:
            cls._LOGGER.info('OFPPortStatus [port=%d] Port deleted.',
                             port.port_no, extra=dpid_str)
            router = cls._ROUTER_LIST[dp_id]


    @classmethod
    def link_down_handler(cls,ev):
        ...
        if dp_id in cls._ROUTER_LIST:
            cls._LOGGER.info('OFPPortStatus [name=%s] [port=%d] Link down.',
                                 port.name, port.port_no, extra=dpid_str)
            router = cls._ROUTER_LIST[dp_id]
        
         
    @classmethod
    def link_up_handler(cls,ev):
        msg = ev.msg
        dp = msg.datapath
        dp_id = dp.id
        dpid_str = {'sw_id': dpid_to_str(dp_id)}
        ...
        if dp_id in cls._ROUTER_LIST:
            cls._LOGGER.info('OFPPortStatus [name=%s] [port=%d] Link up.',
                                 port.name, port.port_no, extra=dpid_str)
            router = cls._ROUTER_LIST[dp_id]

    @classmethod
    def register_path(cls):
        cls._LOGGER.info('Path registered')

    @classmethod
    def unregister_path(cls):
        cls._LOGGER.info('Path unregistered')
        

    @classmethod
    def activate_path(cls):
        cls._LOGGER.info('Path activated')


     # GET /routes/{route_id}
    @rest_command
    def get_route_data(self, req, route_id, **_kwargs):     
        return self._get_routedata(route_id, req)
         

    # POST /routes/{route_id}
    @rest_command
    def set_route_data(self, req, route_id,**_kwargs): 
        return self._set_routedata(route_id,req)


    # DELETE /routes/{route_id}
    @rest_command
    def delete_route_data(self, req, route_id, **_kwargs):
        return self._del_routedata(route_id,req)
  
    # PUT /routes/enable/{route_id}
    @rest_command
    def activate_route_data(self, req, route_id, **_kwargs):
        return self._activate_route_data(route_id, req)

    
    # PUT /routes/disable
    @rest_command
    def disable_route_data(self, req, **_kwargs):
        return self._disable_route_data(req)
    ...
    ## topoproute, sw, gateway are needed data
    ## destination is extra info
